...
public class JWaveZSerialHandler extends InterceptableCallbackHandler {
    ...
    public void addCommandListener(CommandType commandType, JWaveZCommandListener<? extends ZWaveSupportedCommand> listener) {
        LoggingCommandListener<? extends ZWaveSupportedCommand> wrappedListener = new LoggingCommandListener<>(listener);
        this.commandListenersMap.computeIfAbsent(commandType, commandType1 -> new ArrayList<>()).add(wrappedListener);
        this.applicationCommandInterceptor.registerCommandHandler(commandType, wrappedListener);
    }
